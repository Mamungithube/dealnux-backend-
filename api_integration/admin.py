...
class PlatformAdmin(ModelAdmin):
    list_display = ['name', 'code', 'api_enabled', 'listings_count', 'created_at']
...
class ProductListingInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['platform', 'price', 'currency', 'condition', 'seller_username', 'is_available', 'view_link']
...
class ProductAdmin(ModelAdmin):
    list_display = ['image_thumb', 'title', 'category', 'brand', 'lowest_price_display', 'listings_count', 'is_active', 'last_synced']
...
class ProductListingAdmin(ModelAdmin):
    list_display = ['product_title', 'platform', 'price_display', 'condition', 'seller_info', 'quantity', 'is_available', 'last_checked']
    list_filter = ['platform', 'condition', 'is_available', 'returns_accepted', 'free_shipping', 'created_at']
    search_fields = ['product__title', 'external_id', 'seller_username']
    readonly_fields = ['created_at', 'updated_at', 'last_checked', 'total_price_display', 'external_link']
    date_hierarchy = 'created_at'
    ...
    def mark_unavailable(self, request, queryset):
        ...
        self.message_user(request, f'{updated} listings marked as unavailable.')
    
    @admin.action(description='Update prices from API')
    def update_prices(self, request, queryset):
        # TODO: Implement price update logic
        self.message_user(request, f'Price update initiated for {queryset.count()} listings.')
